<?php 
	...
	$result = $conexion->query("SELECT * FROM `pedidos` WHERE estado != 0 AND fecha LIKE '%".$year."%' ORDER BY fecha DESC");
...
<div class="col s12 m12 l12 xl12">
	...
	<div class="col s12 m6 l6 xl4 rubik">
		<div class="z-depth-3 card horizontal card__pad" <?php if ($valor['estado'] == '1') {echo 'style="background-color: #f1c40f"';}else{echo 'style="background-color: #2ecc71"';}?>>
		  <div class="card-stacked">
		    <div class="">
		    	...
		    	<p><div class="dd"><?php echo $valor['fecha']; ?></div></p>

		      	<p style="position: absolute; bottom: 0px; "><b>Total: </b><?php echo 'Bs. '.$valor['total_cd']; ?></p>
		    </div>
		  </div>
		  <div class="card__img">    
		  		<div><a href="#" onclick="detalle('<?php echo $valor['id'] ?>')" style="width: 100%" class="btn waves-effect waves-dark white black-text">Detalle</a></div>
		  		<div <?php if ($valor['estado'] == '1' || $valor['credito'] == '0') {echo 'hidden';}?>><a href="#" onclick="pagos('<?php echo $valor['id'] ?>', '<?php echo $valor['credito'] ?>', '<?php echo $valor['total_cd'] ?>')" style="width: 100%" class="btn waves-effect waves-dark white black-text">Pagos</a></div>
		...
		if (credito == '1') {
			...
			$.ajax({
	            ...
	            success: function(response) {
			        ...
			        let subtotal = 0;
	            	if (res.length < 1) {
	            		$("#pagos tbody").html('<tr><td colspan="2" class="dinamic_rows">No se ha realizado ningún pago...</td></tr>');
	            	}else{
				        for (key in res) {
				            if (res.hasOwnProperty(key)) {
				                ...
				                newRow.textContent = date.getDate()+"-"+(date.getMonth()+1)+"-"+date.getFullYear();
			    	...
			    	document.getElementById('saldo').innerHTML = `Saldo: ${(parseFloat(total_cd)-parseFloat(subtotal)).toFixed(1)}`;
	                $("#modal1").modal('open')
	            },
