...
<body>
    <div class="nav_container">
        <nav>
            <div class="nav-wrapper">
                ...
                <ul class="right">
                    <li><a href="#" id="cart"><i class="grande material-icons"><img style="max-height: 40px;"
                                    src="images/icons/vacio.png" /></i></a></li>
                </ul>
            </div>
        </nav>
        ...
        <ul id="slide-out" class="sidenav roboto">
            <li>
                <div class="user-view">
                    ...
                    <a href="#name"><span
                            class="white-text name"><b><?php echo $_SESSION['usuario']." ".$_SESSION['apellidos']?></b></span></a>
                    <a href="#email"><span
                            class="white-text email"><b><?php echo "CA: ".$_SESSION['ca']?></b></span></a>
                </div>
            </li>
            <li><a href="#!" onclick="window.location.reload()" class="waves-effect waves-teal"><i
                        class="material-icons">home</i>Inicio</a></li>
            <li><a href="#!" onclick="_load(`templates/catalogos/mipedido`)" class="waves-effect waves-teal"><i
                        class="material-icons">shopping_basket</i>Mi pedido</a></li>
            <li><a href="#!" onclick="_load(`templates/catalogos/historial`)" class="waves-effect waves-teal"><i
                        class="material-icons">assignment</i>Historial de pedidos</a></li>
            <li>
            ...
            <li><a class="waves-effect" href="recursos/catalogos/salir.php"><i class="material-icons">logout</i>Cerrar
                    sesión</a></li>
        </ul>
    </div>
	...
	<div class="right_arrow"><a href="#"><i class="large material-icons">chevron_right</i></a></div> -->
        ...
        <div class="container" id="form_container">
            <div class="row">
                ...
                <div class="input-field col s2 " style="text-align: center;">
                    ...
                    <img id="img_prod" src="images/fotos_prod/default.png" alt=""
        ...
        <div class="container center" id="add_container">
            <a class="waves-effect waves-light btn-large shop red lighten-1 fuente" id="add"><i
        ...
        <div class="container">
            <div class="row roboto" id="cart_row" hidden>
                <div class="row get_out">
                    <div class="left">
                        <a href="#!" class="btn-large red lighten-2" id="return"><i
    ...
    $('#search_data').autocomplete({
        ...
        select: function(event, ui) {
            ...
            if (ui.item.pupesos == null) {
                ...
                $("#__datosprod").html("<input id='__datosp' cp='" + ui.item.value + "' np='" + ui
                    .item.id + "' pp='" + ui.item.pupesos + "' fp='" + ui.item.foto + "' st='" +
                    ui.item.cant + "' cl='" + ui.item.codli + "' hidden/>");
...
document.getElementById('add').addEventListener('click', () => {
    ...
    $.ajax({
        ...
        success: function(response) {
            ...
            if (response) {
                return M.toast({
                    html: 'Usted ya tiene un pedido activo.',
                    displayLength: 2000
                })
            } else {
                if (cp == 0) {
                    ...
                    return M.toast({
                        html: "Producto agotado."
                    })
                }
                if (cp == 1) {
                    return M.toast({
                        html: "<span style='color:#ffeb3b'>Debe seleccionar un producto.</span>"
                    })
                }
                if (parseInt(cantp) > parseInt(st)) {
                    return M.toast({
                        html: "<span style='color:#ffeb3b'>Cantidad insuficiente en stock, " +
                            st + " disponibles.</span>"
                    })
                }

                if (parseInt(cantp) > 50 || cantp == "") {
                    M.toast({
                        html: "El pedido no puede superar las 50 unidades"
                    })
                } else {
                    if (parseInt(cantp) < 1 || cantp == "") {
                        M.toast({
                            html: "Ingresa una cantidad válida."
                        })
                    } else {
                        ...
                        Object.keys(reg_pedidos).forEach(function(key) {
                            ...
                            row.insertCell(0).innerHTML =
                                `<a style='text-decotarion: none; cursor: pointer; color: red;' onclick='borrar_prod("${key}")'><i class='material-icons prefix'>delete</i></a>`;
                            row.insertCell(0).innerHTML = reg_pedidos[key][3];
                            row.insertCell(0).innerHTML = reg_pedidos[key][2];
                            row.insertCell(0).innerHTML =
                                `<a href='#' onclick='modal_detalle("${key}", "${reg_pedidos[key][1]}", "${reg_pedidos[key][5]}", "${reg_pedidos[key][4]}")'>${key}</a>`;
                            total = parseFloat(total) + parseFloat(reg_pedidos[key][3]);
                            in_cant = in_cant + parseInt(reg_pedidos[key][2]);
                            if (reg_pedidos[key][7] == '16' || (reg_pedidos[key][7] > 32 &&
                                    reg_pedidos[key][7] < 38)) {
                                total_aux = parseFloat(total_aux) + parseFloat(reg_pedidos[
                                    key][3])
                            } else {
                                total_ped_cd = parseFloat(total_ped_cd) + parseFloat(
                                    reg_pedidos[key][3]);
                            }
                        });
                        total_ped_cd = ((parseFloat(total_ped_cd) * (1 - parseFloat(
                                '<?php echo $_SESSION['desc']; ?>'))) + parseFloat(total_aux))
                            .toFixed(1)
                ...
                M.toast({
                    html: "<span style='color:#1de9b6'>Agregado al carrito de compra.</span>",
                    displayLength: 2500
                })
                $("#__datosprod").html("<input id='__datosp' cp='1' hidden/>")
                $('#search_data').val("")
...
function borrar_prod(x) {
    ...
    total_ped_cd = ((parseFloat(total_ped_cd) * (1 - parseFloat('<?php echo $_SESSION['desc']; ?>'))) + parseFloat(
        total_aux)).toFixed(1)
    $("#total_ped").html(total + " Bs.");
    $("#total_ped_cd").html(total_ped_cd + " Bs.")
    $("#input_total").val(total);
...
document.getElementById('mod_con').addEventListener('click', () => {
    let today = new Date();
    let date = today.getDate() + '-' + ("0" + (today.getMonth() + 1)).slice(-2) + '-' + today.getFullYear() +
    ...
    document.getElementById("conf_cant").innerHTML =
        `<b>Items: </b>${document.getElementById('input_cant').value}`
    // document.getElementById("conf_monto").innerHTML = `<b>Subtotal: </b>${document.getElementById('input_total').value} Bs.`
    document.getElementById("conf_monto_cd").innerHTML =
        `<b>Total: </b>${document.getElementById('input_total_cd').value} Bs.`
...
document.getElementById('conf_ped').addEventListener("click", () => {
    ...
    Object.keys(reg_pedidos).forEach(function(key) {
        ...
        a.push([reg_pedidos[key][0], reg_pedidos[key][1], reg_pedidos[key][2], reg_pedidos[key][3],
            reg_pedidos[key][5], reg_pedidos[key][6], reg_pedidos[key][7]
        ]);
    })
    ...
    if (a.length > 0) {
        $.ajax({
            url: "recursos/catalogos/nuevo_pedido.php?total=" + total + "&total_cd=" + total_cd +
                "&a=" + x + "&cred=" + credito,
            method: "GET",
            success: function(response) {
                ...
                if (response == 1) {
                    M.toast({
                        html: '<span style="color: #2ecc71">Pedido realizado, puedes ver tu pedido en la sección de Mi pedido</span>',
                        displayLength: 5000,
                        classes: 'rounded'
                    })
                    $("#modal2").modal('close')
                    clean_table();
                    let texto =
                        `*LIDER/EXPERTA:*%0A${cliente}%0A*DETALLE DEL PEDIDO:*%0A${detalle}*Monto a pagar:* ${total_cd} Bs.%0A*Tipo de pago:* ${tipo}`;
                ...
                if (response == 2) {
                    M.toast({
                        html: '<span style="color: #f6e58d">Usted ya tiene un pedido pendiente.</span>'
                    })
                }
            },
            error: function(error) {
        ...
        M.toast({
            html: "No se ha seleccionado ningún producto..."
        });
    }
})
...
function clean_table() {
    reg_pedidos = [];
    $("#pedidos_cliente tbody").html(
        "<td colspan=4 class='center'>No se ha seleccionado ningún producto.</td>") //limpiar tabla
    $("#total_ped").html("0 Bs.");
